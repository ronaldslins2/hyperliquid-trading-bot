...
class HyperliquidEndpointRouter:
    ...
    PROVIDER_PRIORITIES = {
        EndpointType.INFO: [Provider.CHAINSTACK, Provider.PUBLIC],      # Prefer Chainstack for data
        EndpointType.EXCHANGE: [Provider.PUBLIC],                       # MUST use public for trading
        EndpointType.EVM: [Provider.CHAINSTACK, Provider.PUBLIC],      # Prefer Chainstack for EVM
        EndpointType.WEBSOCKET: [Provider.CHAINSTACK, Provider.PUBLIC] # Prefer Chainstack for WS
    }
    
    def __init__(self, testnet: bool = True):
        self.testnet = testnet
        self.endpoints: List[EndpointConfig] = []
        self.health_check_interval = int(os.getenv("ENDPOINT_HEALTH_CHECK_INTERVAL", "300"))
        self.health_check_timeout = int(os.getenv("ENDPOINT_HEALTH_CHECK_TIMEOUT", "10"))
    ...
    def _load_endpoints_from_env(self) -> None:
        ...
        for env_suffix, provider, endpoint_type in endpoint_mappings:
            ...
            if url:
                ...
                self.logger.debug(f"Loaded {provider.value} {endpoint_type.value} endpoint: {url[:50]}...")
    ...
    def _load_default_endpoints(self) -> None:
        ...
        if self.testnet:
            defaults = [
                ("https://api.hyperliquid-testnet.xyz/info", Provider.PUBLIC, EndpointType.INFO),
                ("https://api.hyperliquid-testnet.xyz/exchange", Provider.PUBLIC, EndpointType.EXCHANGE),
                ("wss://api.hyperliquid-testnet.xyz/ws", Provider.PUBLIC, EndpointType.WEBSOCKET),
                ("https://api.hyperliquid-testnet.xyz", Provider.PUBLIC, EndpointType.EVM),
            ]
        else:
            defaults = [
                ("https://api.hyperliquid.xyz/info", Provider.PUBLIC, EndpointType.INFO),
                ("https://api.hyperliquid.xyz/exchange", Provider.PUBLIC, EndpointType.EXCHANGE),
                ("wss://api.hyperliquid.xyz/ws", Provider.PUBLIC, EndpointType.WEBSOCKET),
    ...
    def get_endpoint_for_method(self, method_name: str) -> Optional[str]:
        ...
        for endpoint_type in compatible_types:
            ...
            if endpoint:
                self.logger.debug(f"Routing {method_name} to {endpoint.provider.value} {endpoint.endpoint_type.value}")
                return endpoint.url
        
        self.logger.error(f"No healthy endpoints available for method: {method_name}")
        return None
    
    def _get_best_endpoint(self, endpoint_type: EndpointType) -> Optional[EndpointConfig]:
        ...
        if not candidates:
            ...
            if candidates:
                self.logger.warning(f"Using potentially unhealthy endpoint for {endpoint_type.value}")
    ...
    def _start_health_monitoring(self) -> None:
        ...
        self.logger.info(f"Started endpoint health monitoring (interval: {self.health_check_interval}s)")
    ...
    async def _check_endpoint_health(self, endpoint: EndpointConfig) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Health check failed for {endpoint.provider.value} {endpoint.endpoint_type.value}: {e}")
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        for endpoint in self.endpoints:
            status["endpoints"].append({
                ...
                "url": endpoint.url[:50] + "..." if len(endpoint.url) > 50 else endpoint.url,
