...
class AutoPriceRangeConfig:
    ...
    def validate(self) -> None:
        ...
        if self.range_pct < self.min_range_pct or self.range_pct > self.max_range_pct:
            raise ValueError("range_pct must be between min_range_pct and max_range_pct")
...
class PositionSizingConfig:
    ...
    manual: ManualPositionSizingConfig = field(default_factory=ManualPositionSizingConfig)
...
class EnhancedBotConfig:
    ...
    def validate(self) -> None:
        ...
        if self.account.max_allocation_pct > (100.0 - self.grid.position_sizing.auto.balance_reserve_pct):
            raise ValueError(
    ...
    def _validate_private_keys(self) -> None:
        ...
        direct_keys = [self.private_key, self.testnet_private_key, self.mainnet_private_key]
        if any(key is not None for key in direct_keys):
            logger.warning("⚠️  SECURITY WARNING: Private keys found directly in config file!")
            logger.warning("⚠️  Consider using key files or environment variables instead")
        
        # Validate key file paths if specified
        key_files = [self.private_key_file, self.testnet_key_file, self.mainnet_key_file]
            ...
            if key_value is not None:
                ...
                if not (key_value.startswith('0x') and len(key_value) == 66):
                    if not (len(key_value) == 64):  # Allow without 0x prefix
                        logger.warning(f"⚠️  {key_name} may have invalid format (should be 64 hex chars or 0x + 64 hex chars)")
    ...
    def _dataclass_to_dict(self) -> Dict[str, Any]:
        ...
        def convert_value(value, path=""):
            ...
            if isinstance(value, (list, tuple)):
                return [convert_value(item, f"{path}[{i}]") for i, item in enumerate(value)]
            ...
            if hasattr(value, '__dict__'):
                ...
                seen.add(obj_id)
                try:
                    result = {k: convert_value(v, f"{path}.{k}") for k, v in value.__dict__.items()}
                finally:
                    seen.remove(obj_id)
...
def create_default_config(name: str, symbol: str, risk_level: RiskLevel = RiskLevel.MODERATE) -> EnhancedBotConfig:
    ...
    if risk_level == RiskLevel.CONSERVATIVE:
        ...
        auto_price_range = AutoPriceRangeConfig(range_pct=5.0, min_range_pct=3.0, max_range_pct=10.0)
        auto_position_sizing = AutoPositionSizingConfig(balance_reserve_pct=70.0, max_single_position_pct=5.0)
        risk_management = RiskManagementConfig(max_drawdown_pct=10.0, max_position_size_pct=20.0)
        
    elif risk_level == RiskLevel.MODERATE:
        account_config = AccountConfig(max_allocation_pct=20.0, risk_level=risk_level)
        auto_price_range = AutoPriceRangeConfig(range_pct=10.0, min_range_pct=5.0, max_range_pct=20.0)
        auto_position_sizing = AutoPositionSizingConfig(balance_reserve_pct=50.0, max_single_position_pct=10.0)
        risk_management = RiskManagementConfig(max_drawdown_pct=15.0, max_position_size_pct=30.0)
        
    else:  # AGGRESSIVE
        account_config = AccountConfig(max_allocation_pct=40.0, risk_level=risk_level)
        auto_price_range = AutoPriceRangeConfig(range_pct=15.0, min_range_pct=10.0, max_range_pct=30.0)
        auto_position_sizing = AutoPositionSizingConfig(balance_reserve_pct=30.0, max_single_position_pct=20.0)
        risk_management = RiskManagementConfig(max_drawdown_pct=25.0, max_position_size_pct=50.0)
