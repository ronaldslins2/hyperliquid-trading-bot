...
class KeyManager:
    ...
    def get_private_key(self, testnet: bool, bot_config: Optional[Dict[str, Any]] = None) -> str:
        """
        ...
        if legacy_key:
            self.logger.warning(f"Using legacy single private key for {network} - consider using environment-specific keys")
            return legacy_key
        ...
        if legacy_file_key:
            self.logger.warning(f"Using legacy key file for {network} - consider using environment-specific key files")
    ...
    def _get_bot_specific_key(self, bot_config: Dict[str, Any], testnet: bool) -> Optional[str]:
    ...
    def _get_environment_specific_key(self, testnet: bool) -> Optional[str]:
        ...
        env_var = "HYPERLIQUID_TESTNET_PRIVATE_KEY" if testnet else "HYPERLIQUID_MAINNET_PRIVATE_KEY"
    ...
    def _get_file_based_key(self, testnet: bool) -> Optional[str]:
        ...
        env_var = "HYPERLIQUID_TESTNET_KEY_FILE" if testnet else "HYPERLIQUID_MAINNET_KEY_FILE"
    ...
    def get_key_info(self, testnet: bool, bot_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if env_key:
                info.update({
                    "key_source": f"environment_{network}",
                    "key_found": True
                })
                return info
            ...
            if legacy_key:
                info.update({
                    ...
                    "key_found": True,
                    "warnings": ["Using legacy single private key - consider environment-specific keys"]
                })
                return info
            ...
            if legacy_file_key:
                info.update({
                    ...
                    "key_found": True,
                    "warnings": ["Using legacy key file - consider environment-specific key files"]
                })
                return info
