    ...
    def evaluate(
        ...
        for position in positions:
            ...
            if position.entry_price > 0:
                loss_pct = abs(position.unrealized_pnl / (position.entry_price * abs(position.size))) * 100
    ...
    def evaluate(
        ...
        for position in positions:
            ...
            if position.entry_price > 0 and position.unrealized_pnl > 0:
                profit_pct = (position.unrealized_pnl / (position.entry_price * abs(position.size))) * 100
    ...
    def evaluate(
        ...
        for position in positions:
            if account_metrics.total_value > 0:
                position_pct = (position.current_value / account_metrics.total_value) * 100
                ...
                if position_pct >= self.max_position_size_pct:
                    events.append(RiskEvent(
                        ...
                        metadata={
                            ...
                            "suggested_reduction": position_pct - self.max_position_size_pct
                        }
                    ))
...
class RiskManager:
    ...
    def _initialize_rules(self):
        ...
        if risk_config.get("stop_loss_enabled", False):
            self.rules.append(StopLossRule({
                "enabled": True,
                "loss_pct": risk_config.get("stop_loss_pct", 5.0)
            }))
        ...
        # Drawdown rule
        self.rules.append(DrawdownRule({
            "enabled": True,
            "max_drawdown_pct": risk_config.get("max_drawdown_pct", 15.0)
        }))
        
        # Position size rule
        self.rules.append(PositionSizeRule({
            "enabled": True,
            "max_position_size_pct": risk_config.get("max_position_size_pct", 30.0)
        }))
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_events": len([e for e in self.risk_events_history if time.time() - e.timestamp < 3600]),  # Last hour
    ...
    def get_recent_events(self, hours: int = 1) -> List[RiskEvent]:
        ...
        return [event for event in self.risk_events_history if event.timestamp >= cutoff_time]